...
def generateRectangle(width, height):
    v = np.array([[-width / 2, -height / 2, 0],
                  [width / 2, -height / 2, 0],
                  [width / 2, height / 2, 0],
                  [-width / 2, height / 2, 0]],
                 dtype=float)
...
def _CalcContour(V1, V2, V3, C, q, k, n):
    ...
    D = sum(
        [(csi[i] - csi[i - 1]) * np.sinc((ci[i] - ci[i - 1]) / np.pi) * np.exp(-1j * np.pi * (ci[i] + ci[i - 1])) for i
         in range(3)])
...
def CalcRectangle(a, b, theta_arr, phi, ka_arr):
    ...
    for iwave in range(Nw):
        ...
        curve = RCSmaxval * (sinc(ka * np.sin(theta_arr * np.pi / 180)) ** 2)
        curve1 = RCSmaxval * (np.cos(theta_arr * np.pi / 180)**2)
    ...
    if Nw == 1:
        ...
        plt.plot(2, 1/4, marker="o", markersize=5, markeredgecolor="red", markerfacecolor="yellow")
